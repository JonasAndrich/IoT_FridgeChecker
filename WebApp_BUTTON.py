...
def MeanAccess_today(df):
	...
	# im nächsten Schritt kann ich die Öffnungszeiten für einen Tag (yesterday or today) oder die gesamte Periode zusammenzählen.
	...
	# super eleganter Befehl
		...
		return ["Heute", int(opens_today), int(sum_open_today.total_seconds()), int(mean_open_time_today/np.timedelta64(1, 's'))]
	except KeyError:
		print("Der Kühlschrank wurde heute nicht geöffnet!")
		return ["Heute", 0, 0, 0]

def MeanAccess_yesterday(df):
	...
	try:
		...
		return ["Gestern", int(opens_yesterday), int(sum_open_yesterday.total_seconds()), int(mean_open_time_yesterday/np.timedelta64(1, 's')) ]
	except KeyError:
...
def MeanAccess(df):
	...
	return ["Vergangenheit",  int(mean_open_per_days), int(mean_open_time.total_seconds()), int(mean_open_time_all_time/np.timedelta64(1, 's'))]
...
def DataAssembly():
	...
	l1 = MeanAccess_today(df)
	l2 = MeanAccess_yesterday(df)
	l3 = MeanAccess(df)

	l4 = [l1,l2,l3]
	print(l4)
	df_assembled=pd.DataFrame(l4, columns=["Time","Anzahl Öffnungen pro Tag", "Summe Dauer Öffnungen pro Tag [s]", "Mittlere Dauer pro Öffnung [s]"])
...
def serve_layout():
	...
	dcc.Graph(
		...
		figure={
			'data': [
			# HIER TODO -> Y Wert , besser vorher die Groupby series zum Dataframe machen und dann den normalen weg gehen wie oben. 
